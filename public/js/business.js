// business.js v2.0 — 완전 기능 비즈니스 + 펀드레이즈 페이지 (REST API migrated)
(function() {
    'use strict';
    ...
    window.registerBusiness = async function() {
        ...
        try {
            ...
            if (imageFile) {
                ...
                if (uploadResult.url) {
                    businessData.images = [uploadResult.url];
                }
            }

            await fetch('/api/db/businesses', {
                method: 'POST',
    ...
    window.loadBusinesses = async function() {
        ...
        try {
            const res = await fetch('/api/db/businesses?where=status,==,approved&orderBy=createdAt&orderDir=desc&limit=100', { headers: _authHeaders() });
            ...
            <div style="margin-bottom:1.5rem;">
                ...
                <div style="background:#F7F3ED;border:1px solid var(--border);border-radius:8px;padding:1rem;margin-bottom:1rem;">
                    ...
                    <div style="background:#E8E0D8;border-radius:6px;height:10px;overflow:hidden;margin-bottom:0.5rem;">
                        <div style="height:100%;background:${progress >= 100 ? '#5B7B8C' : 'var(--gold,#8B6914)'};width:${progress}%;border-radius:6px;"></div>
                    </div>
    ...
    window.investInBusiness = async function(businessId, businessName) {
        ...
        try {
            ...
            const bizRes = await fetch('/api/db/businesses/' + businessId, { headers: _authHeaders() });
            const businessDoc = await bizRes.json();
            ...
            await fetch('/api/db/businesses/' + businessId, {
                method: 'PUT',
                headers: _authHeaders(),
                body: JSON.stringify({
            ...
            // 투자 기록 저장
            await fetch('/api/db/investments', {
                method: 'POST',
                headers: _authHeaders(),
                body: JSON.stringify({
                    businessId,
    ...
    window.showBusinessQA = async function(businessId, businessName) {
        ...
        try {
            const res = await fetch('/api/db/business_questions?where=businessId,==,' + encodeURIComponent(businessId) + '&where=isPublic,==,true&orderBy=createdAt&orderDir=desc&limit=100', { headers: _authHeaders() });
            const questionsSnap = await res.json();
                    ...
                    questionsHTML += `
                        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:8px;padding:1rem;margin-bottom:1rem;">
                            ...
                            ${q.answered ? `
                                <div style="background:var(--bg);border-left:3px solid var(--gold);padding:0.5rem 1rem;margin-top:0.5rem;">
                                    ...
                                    <div style="font-size:0.7rem;color:var(--accent);margin-top:0.3rem;">
                                        ${t('business.answered_on', 'Answered')}: ${q.answeredAt ? new Date(q.answeredAt).toLocaleDateString() : ''}
                                    </div>
                                </div>
                            ` : `
                            ...
                            <div style="font-size:0.7rem;color:var(--text-muted);margin-top:0.5rem;">
                                ${t('business.asked_on', 'Asked')}: ${q.createdAt ? new Date(q.createdAt).toLocaleDateString() : ''}
                            </div>
                        </div>`;
                });
            }
    ...
    window.askBusinessQuestion = async function(businessId) {
        ...
        try {
            await fetch('/api/db/business_questions', {
                method: 'POST',
                headers: _authHeaders(),
                body: JSON.stringify({
                    businessId,
    ...
    window.answerBusinessQuestion = async function(questionId, businessId) {
        ...
        try {
            ...
            await fetch('/api/db/business_questions/' + questionId, {
                method: 'PUT',
                headers: _authHeaders(),
                body: JSON.stringify({
                    answer: answer.trim(),
    ...
    window.loadCampaigns = async function() {
        ...
        try {
            let queryUrl = '/api/db/campaigns?orderBy=createdAt&orderDir=desc&limit=30';
            if (interestFilter !== 'all' && interestFilter !== 'best') {
                queryUrl += '&where=category,==,' + encodeURIComponent(interestFilter);
            }
            const res = await fetch(queryUrl, { headers: _authHeaders() });
            const snap = await res.json();
            list.innerHTML = '';
                ...
                card.innerHTML = `
                    ...
                    <div style="padding:1rem;">
                        ...
                        <div style="background:var(--bg,#0a0a1a);border-radius:4px;height:8px;overflow:hidden;margin-bottom:0.5rem;">
                            <div style="height:100%;background:${progress >= 100 ? '#5B7B8C' : 'var(--gold,#8B6914)'};width:${progress}%;border-radius:4px;transition:width 0.3s;"></div>
                        </div>
    ...
    window.showCampaignDetail = function(id, data) {
        ...
        content.innerHTML = `
            ...
            <div style="background:#F7F3ED;border-radius:6px;height:10px;overflow:hidden;margin-bottom:0.5rem;">
                <div style="height:100%;background:${progress >= 100 ? '#5B7B8C' : '#8B6914'};width:${progress}%;border-radius:6px;"></div>
            </div>
