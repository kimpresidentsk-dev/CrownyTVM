...
impl TritVM {
    pub fn new(code: Vec<Instruction>, constants: Vec<ConstValue>) -> Self {
        TritVM {
            ...
            cycles: 0, output: Vec::new(), trace: false, max_cycles: 1_000_000,
        }
    }
    ...
    pub fn step(&mut self) -> Result<(), VmError> {
        ...
        match opc {
            ...
            // 주장 생성: [스택: 대상, 술어, 값, 인식상태(0~3)] → Claim
            op::CLAIM_NEW => {
            ...
            // 인식상태 읽기: [Claim] → Trit (Ti=확정, Om=미확인, Ta=오해)
            op::CLAIM_STATE => {
                let v = self.pop()?;
            ...
            // 의사결정 점수: [Claim, 목표적합도(f64)] → Float
            // score = goal_fit × evidence_quality × state_reliability
            op::CLAIM_DECIDE => {
            ...
            op::BUILTIN => {
                ...
                match id {
                    // ─── 수학 (0~19) ───
                    0 => { let v = self.pop()?; self.stack.push(Value::Int(v.to_float().round() as i64)); }    // 반올림
                    1 => { let v = self.pop()?; self.stack.push(Value::Int(v.to_float().floor() as i64)); }    // 바닥
                    2 => { let v = self.pop()?; self.stack.push(Value::Int(v.to_float().ceil() as i64)); }     // 천장
                    3 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().sin())); }           // 사인
                    4 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().cos())); }           // 코사인
                    5 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().tan())); }           // 탄젠트
                    6 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().sqrt())); }          // 제곱근
                    7 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().abs())); }           // 절대값f
                    8 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().ln())); }            // 자연로그
                    9 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float().log10())); }         // 상용로그
                    // ─── 시간 (10~14) ───
                    10 => {  // 현재시간 (초)
                    ...
                    // ─── 타입 (15~19) ───
                    15 => { let v = self.pop()?; self.stack.push(Value::Str(v.type_name().to_string())); }     // 타입이름
                    16 => { let v = self.pop()?; self.stack.push(Value::Trit(if matches!(v, Value::Int(_)) { Trit::Ti } else { Trit::Ta })); }  // 정수?
                    17 => { let v = self.pop()?; self.stack.push(Value::Trit(if matches!(v, Value::Float(_)) { Trit::Ti } else { Trit::Ta })); } // 실수?
                    18 => { let v = self.pop()?; self.stack.push(Value::Trit(if matches!(v, Value::Str(_)) { Trit::Ti } else { Trit::Ta })); }   // 문자열?
                    19 => { let v = self.pop()?; self.stack.push(Value::Trit(if matches!(v, Value::Array(_)) { Trit::Ti } else { Trit::Ta })); } // 배열?
                    // ─── JSON (20~21) ───
                    20 => {  // JSON 파싱
                    ...
                    31 => { let v = self.pop()?; self.stack.push(Value::Str(v.to_string_val().to_uppercase())); }          // 대문자
                    32 => { let v = self.pop()?; self.stack.push(Value::Str(v.to_string_val().to_lowercase())); }          // 소문자
                    33 => { let v = self.pop()?; self.stack.push(Value::Str(v.to_string_val().trim().to_string())); }      // 공백제거
                    34 => { let v = self.pop()?; self.stack.push(Value::Str(v.to_string_val().chars().rev().collect())); }  // 뒤집기
                    35 => {  // 나누기(문자열, 구분자)
                    ...
                    // ─── 변환 (50~54) ───
                    50 => { let v = self.pop()?; self.stack.push(Value::Int(v.to_int())); }      // →정수
                    51 => { let v = self.pop()?; self.stack.push(Value::Float(v.to_float())); }  // →실수
                    52 => { let v = self.pop()?; self.stack.push(Value::Str(v.to_string_val())); } // →문자열
                    53 => { let v = self.pop()?; self.stack.push(Value::Trit(v.is_truthy())); }  // →트릿
                    // ─── 랜덤 (60) ───
                    60 => {  // 랜덤(0.0~1.0) — 간단한 LCG
