...
impl Parser {
    ...
    fn parse_import(&mut self) -> Result<Stmt, String> {
        self.advance(); // 가져오기
        match self.cur().clone() {
            Token::StrLit(s) => { self.advance(); Ok(Stmt::Import(s)) }
            t => Err(format!("가져오기 뒤에 문자열 기대, 실제: {:?}", t))
        }
    }
    ...
    fn parse_ident_stmt(&mut self) -> Result<Stmt, String> {
        ...
        } else if matches!(self.cur(), Token::LBrack) {
            // 배열 인덱스 대입: a[i] = v
            self.advance(); // [
    ...
    fn parse_if3(&mut self) -> Result<Stmt, String> {
        ...
        for _ in 0..3 {
            let is_ti = matches!(self.cur(), Token::TritTi) || matches!(self.cur(), Token::Ident(ref s) if s == "긍정");
            let is_om = matches!(self.cur(), Token::TritOm) || matches!(self.cur(), Token::Ident(ref s) if s == "보류");
            let is_ta = matches!(self.cur(), Token::TritTa) || matches!(self.cur(), Token::Ident(ref s) if s == "부정");
            if is_ti {
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        match self.cur().clone() {
            ...
            Token::KwBuiltin => {
                ...
                self.expect(&Token::RParen)?;
                Ok(Expr::Builtin { id: Box::new(id), args })
            }

            // Stage 1: __sys__("name", args...)
            Token::KwSyscall => {
                self.advance();
                self.expect(&Token::LParen)?;
                ...
                self.expect(&Token::RParen)?;
                Ok(Expr::Syscall { name: Box::new(name), args })
            }
