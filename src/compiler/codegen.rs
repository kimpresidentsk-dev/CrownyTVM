...
pub struct CodeGen {
    ...
    /// 상수 인라인: 상수명 → 리터럴 값 (함수 내부에서 전역 상수 접근 허용)
    ...
    fn gen_stmt(&mut self, stmt: &Stmt) -> Result<(), String> {
        match stmt {
            ...
            Stmt::Const { name, value } => {
                // 리터럴 상수는 인라인 (함수 내부에서 전역 상수 접근 가능)
                // 음수(-4 등)는 UnaryOp{Neg, IntLit}로 파싱되므로 이것도 인라인
                let is_literal = matches!(value, Expr::IntLit(_) | Expr::FloatLit(_) | Expr::StrLit(_) | Expr::TritLit(_))
                    || matches!(value, Expr::UnaryOp { op: UnaryOp::Neg, expr } if matches!(expr.as_ref(), Expr::IntLit(_) | Expr::FloatLit(_)));
                if is_literal {
            ...
            Stmt::Import(_) => { /* 컴파일러/mod.rs에서 선처리됨 */ }
            Stmt::Export(_) => { /* 메타데이터만 — 코드 생성 불필요 */ }
    ...
    fn gen_expr(&mut self, expr: &Expr) -> Result<(), String> {
        match expr {
            ...
            Expr::Builtin { id, args } => {
                // 인자를 스택에 푸시
                for arg in args { self.gen_expr(arg)?; }
                // id는 Box<Expr>이므로 정수값 추출
                let builtin_id = match id.as_ref() {
                    Expr::IntLit(v) => *v as i32,
                    _ => return Err("__내장__ id는 정수여야 합니다".into()),
                };
                self.emit_a(op::BUILTIN, builtin_id);
            }

            Expr::Syscall { name, args } => {
                // 인자 → 스택, 이름 → 스택, SYSCALL(인자수)
                for arg in args { self.gen_expr(arg)?; }
                // name은 Box<Expr>이므로 문자열값 추출
                match name.as_ref() {
                    Expr::StrLit(s) => self.push_str(s),
                    _ => { self.gen_expr(name)?; }
                }
